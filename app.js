...
window.delRow=(type,i)=>{if(type==='staff'){staff.splice(i,1);store.set('waslah_staff',staff)}if(type==='parents'){parents.splice(i,1);store.set('waslah_parents',parents)}if(type==='requests'){requests.splice(i,1);store.set('waslah_requests',requests)}renderAll();toast('تم الحذف')};
